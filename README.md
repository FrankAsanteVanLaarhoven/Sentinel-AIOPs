...
| Benchmark (multi-source) | `causal_root` *(metrics-only)* | RCAEval RE2 (OB+SS+TT) | **AC@1 0.815** / **Avg@5 0.873** across **270 cases** — *framing A: metric channel only*; beats metric-only **BARO** on AC@1 on all 3 (OB 0.91 vs 0.78 · SS 0.88 vs 0.81 · TT 0.66 vs 0.59); TT is graph-free — a call graph *derived from RE2 traces* was measured (helps RE2-TT +3.3, hurts the RE1 headline −6.4 → graph-free kept; `make tt-graph-delta`) |
| Benchmark (code-level) | `causal_root` *(metrics-only)* | RCAEval RE3 (OB+SS+TT) | **AC@1 0.494** / Avg@5 0.655 across **87 cases** (f1–f5 code faults) — *honestly outperformed* by metric-only **BARO** (0.784 / 0.909); the causal rule's ceiling on subtle faults — an ablation confirms robust/soft elevation does **not** close it (the gap is BARO's magnitude-only ranking vs the dependency demotion, §7.2f). Marks the metrics-only envelope; multi-source is Paper 2 |
...
make validate-rcaeval# RCAEval RE1 (OB+SS+TT, 375 cases) → selective Top-1 0.845 / Top-3 0.912 (deterministic causal_root)
