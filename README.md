...
- Endpoint reference and model cards: `GET /investigate`, `/action-proposal` (typed VerdictPlane hand-off), `/audit` + `/audit/verify` (tamper-evident proposal provenance), `/validation`, `/log-anomaly`, `/metric-anomaly`, `/rca-validation`, `/rcaeval`.
