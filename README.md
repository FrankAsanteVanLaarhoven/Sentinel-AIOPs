...
| Benchmark (code-level) | `causal_root` *(metrics-only)* | RCAEval RE3 (OB+SS+TT) | **AC@1 0.494** / Avg@5 0.655 across **87 cases** (f1–f5 code faults) — *honestly outperformed* by metric-only **BARO** (0.784 / 0.909); the z≥3 elevation rule's ceiling on subtle faults (§7.2f). Marks the metrics-only envelope; multi-source is Paper 2 |
